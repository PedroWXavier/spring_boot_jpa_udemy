...
public class AutorController {
    ...
    public ResponseEntity<AutorResponseDTO> obterDetalhes(@PathVariable("id") String id) {
        ...
        if (autorOptional.isPresent()) {
            ...
            var dto = new AutorResponseDTO(autor.getId(), autor.getNome(), autor.getDataNascimento(), autor.getNacionalidade());
    ...
    @PutMapping("{id}")
    public ResponseEntity<Object> atualizar(@PathVariable("id") String id, @RequestBody AutorRequestDTO autorDTO) {
        try {
            var idAutor = UUID.fromString(id);
            Optional<Autor> autorOptional = service.obterPorId(idAutor);
